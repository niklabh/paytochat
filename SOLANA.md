...
- [ ] Rebuild so the new ID is baked into the `.so`:
      `anchor build`
- [ ] **Back up the program keypair offline** (1Password attachment +
      a hardware token like a YubiKey). Losing it permanently locks
      out future upgrades on this Program ID.
- [ ] Commit the synced `Anchor.toml` and `lib.rs` so production builds
...
- [ ] All 12 tests must pass. Anything red blocks deploy.
...
You'll initialize from a Node script using the IDL Anchor just emitted.
Below assumes you've created `solana/scripts/initialize.ts` (snippet
provided after the checklist) — adjust paths as needed.
...
Reference `initialize.ts` (you'll author this once and reuse for
devnet + mainnet):

```ts
import * as anchor from "@coral-xyz/anchor";
import { PublicKey, SystemProgram } from "@solana/web3.js";
import idl from "../target/idl/paytochat_escrow.json";

const provider = anchor.AnchorProvider.env(); // honors ANCHOR_PROVIDER_URL + ANCHOR_WALLET
anchor.setProvider(provider);
const program = new anchor.Program(idl as anchor.Idl, provider);

const admin = new PublicKey(process.env.ADMIN_PUBKEY!);
...
- [ ] Run `initialize.ts` against mainnet, signed by the deployer,
      with the Squads vault as `admin`:
      ```bash
...
- [ ] Copy `solana/target/idl/paytochat_escrow.json` into
      `src/lib/payments/sol-escrow-idl.json`.
- [ ] Copy `solana/target/types/paytochat_escrow.ts` into
      `src/lib/payments/sol-escrow-types.ts` so client + server share
      typed `program.methods.*`.
- [ ] Add a small build script (mirror the EVM
      `contracts/scripts/export-abi.ts`) that re-runs the copy after
      every `anchor build`:
      ```ts
      // solana/scripts/export-idl.ts
      ...
      console.log("synced IDL + types into src/lib/payments/");
      ```
